 ...
 * Surge：
[Script]
彩云天气SVIP = type=http-response,requires-body=1,max-size=0,pattern=^http[s]?:\/\/biz\.(caiyunapp|cyapi)\.(com|cn)\/(membership_rights|v2\/user),script-path=https://raw.githubusercontent.com/Tartarus2014/Script/master/CaiYun.js
 *
 * Loon：
[Script]
http-response ^http[s]?:\/\/biz\.(caiyunapp|cyapi)\.(com|cn)\/(membership_rights|v2\/user),script-path=https://raw.githubusercontent.com/Tartarus2014/Script/master/CaiYun.js,requires-body=1, tag=彩云天气SVIP
 */


const SCRIPT_NAME = '彩云天气';
const USER_REGEX = /http[s]?:\/\/biz\.(caiyunapp|cyapi)\.(com|cn)\/v2\/user/;
const RIGHTS_REGEX = /http[s]?:\/\/biz\.(caiyunapp|cyapi)\.(com|cn)\/membership_rights/;
const RESULT = {
  is_vip: true,
  vip_type: "s",
